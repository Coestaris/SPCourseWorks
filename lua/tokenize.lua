...
function tokenize.proceed(filename, storage, et1_print, next_et_function, et2_print)
    ...
    for i, line in pairs(lines) do
        ...
        local structure = {}
        if not asm_storage.has_error(i, storage) then
            structure = get_structure(token_types, tokens)
        end;

        if et1_print and not asm_storage.has_error(i, storage) then
            structure:print()
            print()
        end
...
--
function tokenize.output(storage)
    assert(type(storage) == "table")
    print("Et1 table")
end
